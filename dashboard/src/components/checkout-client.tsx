...
export function CheckoutClient() {
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      if (!active) return;
      try {
        const draft = localStorage.getItem("nest-shop-checkout-draft");
  ...
  useEffect(() => {
    localStorage.setItem("nest-shop-checkout-draft", JSON.stringify(details));
  }, [details]);
  ...
  const totalMinor = subtotalMinor + (selectedShipping?.priceMinor ?? 0);
  ...
  async function placeOrder(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      clearCart();
      localStorage.removeItem("nest-shop-checkout-draft");
    ...
    <main className="checkout-page">
      ...
      <form className="checkout-layout" onSubmit={placeOrder}>
        <div className="checkout-fields">
          ...
          <fieldset><legend>Promotion</legend><label>Promotion code<input value={promotionCode} onChange={(event) => setPromotionCode(event.target.value.toUpperCase())} placeholder="WELCOME10" autoComplete="off" /><small>Eligible discounts are applied when you place your order.</small></label></fieldset>
        ...
        <aside className="checkout-summary"><p className="shop-kicker">Your order</p><h2>{cart.reduce((sum, line) => sum + line.quantity, 0)} items</h2><div className="checkout-summary-lines">{cart.map((line) => <div key={line.id}><span>{line.quantity} × {line.name}</span><strong>{money(line.priceMinor * line.quantity, line.currency)}</strong></div>)}</div><dl><div><dt>Subtotal</dt><dd>{money(subtotalMinor)}</dd></div><div><dt>Delivery</dt><dd>{selectedShipping ? (selectedShipping.priceMinor ? money(selectedShipping.priceMinor) : "Included") : "—"}</dd></div></dl><div className="checkout-total"><span>Total</span><strong>{money(totalMinor)}</strong></div>{error && <p className="form-error" role="alert">{error}</p>}<button className="shop-primary" disabled={submitting || !shippingMethod}>{submitting ? "Placing order…" : `Place order · ${money(totalMinor)}`}</button><Link href="/shop/bag">← Back to bag</Link></aside>
      </form>
    </main>
  );
}
